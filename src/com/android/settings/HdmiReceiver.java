...
public class HdmiReceiver extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent){
	    ...
	    int scale=0;
	    int resol=0;
	    preferences = context.getSharedPreferences("HdmiSettings", Context.MODE_PRIVATE);
	    if (intent.getAction().equals(ACTION)){
		    if(SystemProperties.get("ro.board.platform","none").equals("rk29xx"))
		    {
			    enable = preferences.getInt("enable", 1);
			    scale = preferences.getInt("scale_set", 100);
			    resol = preferences.getInt("resolution", 3);
		    ...
		    }else
		    {
			    ...
			    if(state==1){
				    enable = preferences.getInt("enable", 1);
				    resol = preferences.getInt("resolution", 2);
